...
class Fast_iTPN(nn.Module):
    ...
    def search_subspace_distance(self, q_search, subspace_num, T_miu, T_U, k):
        ...
        for i in range(combined_x.shape[0]):
            ...
            distMi = (T_miu - miu_xi).unsqueeze(0) @ (
                    2 * torch.eye(512).cuda() - T_U @ T_U.T - U_xi @ U_xi.T
            ) @ (T_miu - miu_xi)
    ...
    def search_subspace_distance_Eu(self, q_search, subspace_num, T_miu, T_U, k):
        combined_x = torch.stack(list(q_search)[-subspace_num:], dim=0).squeeze(1).transpose(0, 2)
        S_miu, S_U, S_sigma, dist_list = [], [], [], []
        ...
        for i in range(combined_x.shape[0]):
            ...
            distGi = torch.norm(T_U - U_xi, p='fro')
            distMi = (T_miu - miu_xi).unsqueeze(0) @ (
                    2 * torch.eye(512).cuda() - T_U @ T_U.T - U_xi @ U_xi.T
            ) @ (T_miu - miu_xi)
    ...
    def search_subspace_distance_sin(self, q_search, subspace_num, T_miu, T_U, k):
        ...
        for i in range(combined_x.shape[0]):
            ...
            disti = 0.9999 * distPi + 0.0001 * distMi
            # disti = distPi
            dist_list.append(disti)
    ...
    def incremental_search_subspace(self, q_search, subspace_num, S_U, S_sigma, S_miu, T_U, T_miu, r, k):
        ...
        for i in range(B1.shape[0]):
            ...
            distmi = (T_miu - mius_update).unsqueeze(0) @ (2 * torch.eye(512).cuda() - T_U @ T_U.T - us_update @ us_update.T) @ (
                    T_miu - mius_update)
            dist_i = 0.999 * distgi + 0.001 * distmi
            # dist_i = distgi
    ...
    def incremental_search_subspace_Eu(self, q_search, subspace_num, S_U, S_sigma, S_miu, T_U, T_miu, r, k):
        dist_list = []
        ...
        for i in range(B1.shape[0]):
            ...
            R = torch.cat((R_upper, R_lower), dim=0)
            u, s, v = torch.linalg.svd(R)  # 使用torch.linalg.svd代替torch.linalg.svd
            u = u[:, :k]
            ...
            distgi = torch.norm(T_U - us_update, p='fro')
            distmi = (T_miu - mius_update).unsqueeze(0) @ (2 * torch.eye(512).cuda() - T_U @ T_U.T - us_update @ us_update.T) @ (
                    T_miu - mius_update)
    ...
    def incremental_search_subspace_sin(self, q_search, subspace_num, S_U, S_sigma, S_miu, T_U, T_miu, r, k):
        ...
        for i in range(B1.shape[0]):
            ...
            distmi = (T_miu - mius_update).unsqueeze(0) @ (2 * torch.eye(512).cuda() - T_U @ T_U.T - us_update @ us_update.T) @ (
                    T_miu - mius_update)
            dist_i = 0.9999 * distpi + 0.0001 * distmi
            # dist_i = distpi
            dist_list.append(dist_i)
    ...
    def principal_angles(self, u1, u2):
        ...
        Pu, Sigma, Pv = torch.linalg.svd(T)
        cos_theta = Sigma
    ...
    def prepare_tokens_with_masks_gr(self, template_list, search_list, template_anno_list, time=None, r_s=None, r_t=None, update=None, subspace_num=None):
        self.frame += 1
        self.time -= 1
        ...
        max_pool = nn.MaxPool1d(kernel_size=49, stride=49)
        ...
        if (self.frame - 1) % update == 0 and self.frame != 1:
            o_reshaped = z.view(5, 512, -1)

            for i in range(o_reshaped.shape[0]):

                o_i = o_reshaped[i].unsqueeze(0)

                o_pooled = max_pool(o_i.detach())
                self.q_online.enqueue(o_pooled)
            self.T_U, self.T_sigma, self.T_miu = self.incremental_template_subspace(
                self.q_online, subspace_num, self.T_U, self.T_sigma, self.T_miu, r_t, self.k
            )
            if self.q_search.is_full():
                self.S_U, self.S_sigma, self.S_miu, self.dist_map = self.incremental_search_subspace(
                    self.q_search, subspace_num, self.S_U, self.S_sigma, self.S_miu, self.T_U, self.T_miu, r_s, self.k
                )

                self.time = time
            self.q_online.clear()

        if self.T_U is None and self.q_template.size() == subspace_num:
            ...
            self.T_miu, self.T_U, self.T_sigma = self.compute_template_subsapce(self.q_template, subspace_num, self.k)

        if self.q_search.size() == subspace_num and not self.S_U:
            ...
            #     self.q_search, subspace_num, self.T_miu, self.T_U, self.k)
            #
        self.q_search.enqueue(x.reshape(1, 512, -1))
        if self.time == 0:
            self.dist_map = torch.ones([self.dim]).cuda()

        # x_s_gr = self.dist_map.view(1, self.dim, 1) * x_s
    ...
    def forward_features_gr(self, template_list, search_list,template_anno_list, time=None, r_s=None, r_t=None, update=None, subspace_num=None):
        xz_gr = self.prepare_tokens_with_masks_gr(template_list, search_list, template_anno_list,
                                                  time=time, r_s=r_s, r_t=r_t, update=update, subspace_num=subspace_num)
        xz_gr = self.pos_drop(xz_gr)
        return xz_gr
    ...
    def forward_gr(self, template_list,search_list,template_anno_list, time=None, r_s=None, r_t=None, update=None, subspace_num=None):
        xz_gr = self.forward_features_gr(template_list,search_list,template_anno_list,
                                         time=time, r_s=r_s, r_t=r_t, update=update, subspace_num=subspace_num)
        return xz_gr
...
def fastitpnl(pretrained=False,pos_type="interpolate",pretrain_type="", **kwargs):
    model = Fast_iTPN(
        patch_size=16, embed_dim=512, depth_stage1=2, depth_stage2=2, depth=40, num_heads=12, bridge_mlp_ratio=3.,
        mlp_ratio=3., qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
        convmlp=True,
        naiveswiglu=True,
        subln=True,
