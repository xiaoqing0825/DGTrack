...
class BaseBackbone(nn.Module):
    ...
    def forward_gr(self, z, x, **kwargs):
        """
        Joint feature extraction and relation modeling for the basic ViT backbone.
        Args:
            z (torch.Tensor): template feature, [B, C, H_z, W_z]
        ...
        """
        if "token_type" in kwargs.keys():
            x, aux_dict = self.forward_features_gr(z, x, track_query=kwargs['track_query'], token_type=kwargs['token_type'])
        else:
            x, aux_dict = self.forward_features_gr(z, x, track_query=kwargs['track_query'])
