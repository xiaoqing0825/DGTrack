...
class ODTrack(nn.Module):
    ...
    def forward(self, template: torch.Tensor,
        ...
        for i in range(len(search)):
            x, aux_dict = self.backbone.forward(z=template.copy(), x=search[i],
    ...
    def forward_gr(self, template: torch.Tensor,
                search: torch.Tensor,
                ce_template_mask=None,
                ce_keep_rate=None,
                return_last_attn=False, time=None, r_s=None, r_t=None, update=None,
                   s_num=None, t_num=None, sub_template=None, show_sub=None
                ):
        assert isinstance(search, list), "The type of search is not List"

        out_dict = []
        for i in range(len(search)):
            x, aux_dict = self.backbone.forward_gr(z=template.copy(), x=search[i],
                                        ce_template_mask=ce_template_mask, ce_keep_rate=ce_keep_rate,
                                        return_last_attn=return_last_attn, track_query=self.track_query,
                                        token_len=self.token_len, time=time, r_s=r_s, r_t=r_t, update=update, s_num=s_num,
